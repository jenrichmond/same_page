...
# sample size
sample_size_data_A = final1A %>% group_by(subfield_groups) %>% summarize(num=n(), total_data_score)

dataplot1 <- final1A %>%  
  left_join(sample_size_data_A) %>%
  mutate(myaxis = paste0(subfield_groups, "\n", "n=", num)) %>%
  ggplot(aes(x = myaxis, y = total_data_score, fill = subfield_groups)) +
  geom_flat_violin(position = position_nudge(x = 0, y = 0),adjust = 0.5, alpha = 0.5) +
  gghalves::geom_half_point(
    ...
    alpha = .3, position = position_nudge(x = -0.05, y = 0)) +
    theme_classic() +
...
```{r}
# sample size
sample_size_materials_A = final1A %>% group_by(subfield_groups) %>% summarize(num=n())

materialsplot3 <- final1A %>%  
  left_join(sample_size_materials_A) %>%
  mutate(myaxis = paste0(subfield_groups, "\n", "n=", num)) %>%
  ggplot(aes(x = myaxis, y = total_materials_score, fill = subfield_groups)) +
  geom_flat_violin(position = position_nudge(x = 0, y = 0),adjust = 0.5, alpha = 0.5) +
  gghalves::geom_half_point(
    ## draw jitter on the left
    side = "l", 
