...
data_plot <- ggplot(all_data_stats, aes(y=Percent, group = subfield_groups, colour = subfield_groups)) +
  ...
  geom_point(aes(x=Real_Stage), size=8, shape=21, fill="white", position = position_dodge(width=0)) +
...
```{r echo=FALSE}
# position dodge is at width = 0.75
...
# Using pointlocation (from Kidwell OSF)

pointlocation <- c(1.28,2,3,4,5,1.1,2,3,4,4.95,.92,2.09,3,4,5.07,.74,1.92,3,4,5)

ggplot(all_material_stats, aes(y=Percent,  group = subfield_groups, colour = subfield_groups)) +
  coord_cartesian(ylim=c(0,100)) +
  geom_line(aes(size=subfield_groups, colour=subfield_groups, x=Real_Stage)) +
