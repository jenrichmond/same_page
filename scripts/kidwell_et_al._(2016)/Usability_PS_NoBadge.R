...
## THIS SCRIPT BREAKS DOWN THE USABILITY OF DATA AND MATERIALS IN PSYCHOLOGICAL SCIENCE THAT WERE REPORTEDLY AVAILABLE AT OPEN ACCESS LOCATIONS BUT NOT AWARDED A BADGE.
...
keeps <- c("year", "Are.the.data.understandable.and.usable.after.brief.review.", "No") # Change to columns you want to keep
keepcolumns <- withtotal[keeps]
Usability.NoDataBadge <- spread(keepcolumns, Are.the.data.understandable.and.usable.after.brief.review., No)
Usability.NoDataBadge[is.na(Usability.NoDataBadge)] <- 0

colnames(Usability.NoDataBadge)[2] <- "N/A" # Manual curation

# At this point you should have 5 columns
colnames(Usability.NoDataBadge) #  puts out the order so you can rearrange
Usability.NoDataBadge <- Usability.NoDataBadge[c(1,5,3,4,2)] # Rearrange columns so they match the order you want
Before_badges <- colSums(Usability.NoDataBadge[1:2, c(2,3,4,5)])
Before_badges <- c("2012-2013", Before_badges)
After_badges <- colSums(Usability.NoDataBadge[3:4, c(2,3,4,5)])
After_badges <- c("2014-2015", After_badges)
Usability.NoDataBadge <- rbind(Usability.NoDataBadge, Before_badges, After_badges)

#Open dataframe "Usability.NoDataBadge" for final output

########## USABILITY: NO MATERIALS BADGE ##############
...
keeps <- c("year", "Are.the.materials.understandable.and.usable.after.brief.review.", "No") # Change to columns you want to keep
keepcolumns <- withtotal[keeps]
Usability.NoMaterialsBadge <- spread(keepcolumns, Are.the.materials.understandable.and.usable.after.brief.review., No)
Usability.NoMaterialsBadge[is.na(Usability.NoMaterialsBadge)] <- 0

colnames(Usability.NoMaterialsBadge)[2] <- "N/A" # Manual curation

# At this point you should have 5 columns
colnames(Usability.NoMaterialsBadge) #  puts out the order so you can rearrange
Usability.NoMaterialsBadge <- Usability.NoMaterialsBadge[c(1,5,3,4,2)] # Rearrange columns so they match the order you want
Before_badges <- colSums(Usability.NoMaterialsBadge[1:2, c(2,3,4,5)])
Before_badges <- c("2012-2013", Before_badges)
After_badges <- colSums(Usability.NoMaterialsBadge[3:4, c(2,3,4,5)])
After_badges <- c("2014-2015", After_badges)
Usability.NoMaterialsBadge <- rbind(Usability.NoMaterialsBadge, Before_badges, After_badges)

#Open dataframe "Usability.NoMaterialsBadge" for final output