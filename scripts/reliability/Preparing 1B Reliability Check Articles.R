...
# pull 1B data
data1B  <- fetch_survey(surveyID = "SV_6nAyR8nkKxepwxw", verbose = TRUE,force_request = TRUE) %>%
  clean_names()

#filter data1B to select only variables of interest
data1B_select <- data1B %>%
  select(q2, q4_1, q9:q50)

# let's rename the variables
data1B_rename <- data1B_select %>%
  rename(coder_name = q2, 
...
sep1B <- data1B_rename %>%
  separate(q4_1, into = "article_id_number", sep = "\\s", remove = FALSE) %>%
...
sep1B %>% write_csv(here::here("data_files", "data1B_reliability_checking"))
