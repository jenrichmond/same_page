...
## THIS SCRIPT BREAKS DOWN THE ACTUAL AVAILABILITY OF DATA AND MATERIALS IN PSYCHOLOGICAL SCIENCE THAT WERE REPORTEDLY AVAILABLE AT OPEN ACCESS LOCATIONS BUT NOT AWARDED A BADGE.
...
########### ACTUAL AVAILABILITY: NO DATA BADGE ##############
...
keeps <- c("year", "Are.the.data.located.at.the.working.page.", "No") # Change to columns you want to keep
keepcolumns <- withtotal[keeps]
ActualAvailability.NoDataBadge <- spread(keepcolumns, Are.the.data.located.at.the.working.page., No)
ActualAvailability.NoDataBadge[is.na(ActualAvailability.NoDataBadge)] <- 0

ActualAvailability.NoDataBadge[,"N/A"] <- 0 # Manual curation, adding known zeroes

# At this point you should have 6 columns
colnames(ActualAvailability.NoDataBadge) #  puts out the order so you can rearrange
ActualAvailability.NoDataBadge <- ActualAvailability.NoDataBadge[c(1,4,2,3,5)] # Rearrange columns so they match the order you want
Before_badges <- colSums(ActualAvailability.NoDataBadge[1:2, c(2,3,4,5)])
Before_badges <- c("2012-2013", Before_badges)
After_badges <- colSums(ActualAvailability.NoDataBadge[3:4, c(2,3,4,5)])
After_badges <- c("2014-2015", After_badges)
ActualAvailability.NoDataBadge <- rbind(ActualAvailability.NoDataBadge, Before_badges, After_badges)

## Open dataframe "ActualAvailability.NoDataBadge" for final output

########## ACTUAL AVAILABILITY: NO MATERIALS BADGE ##############
...
keeps <- c("year", "Are.the.materials.located.at.the.working.page.", "No") # Change to columns you want to keep
keepcolumns <- withtotal[keeps]
ActualAvailability.NoMaterialsBadge <- spread(keepcolumns, Are.the.materials.located.at.the.working.page., No)
ActualAvailability.NoMaterialsBadge[is.na(ActualAvailability.NoMaterialsBadge)] <- 0

colnames(ActualAvailability.NoMaterialsBadge)[2] <- "N/A" # Manual curation

# At this point you should have 5 columns
colnames(ActualAvailability.NoMaterialsBadge) #  puts out the order so you can rearrange
ActualAvailability.NoMaterialsBadge <- ActualAvailability.NoMaterialsBadge[c(1,5,4,3,2)] # Rearrange columns so they match the order you want
Before_badges <- colSums(ActualAvailability.NoMaterialsBadge[1:2, c(2,3,4,5)])
Before_badges <- c("2012-2013", Before_badges)
After_badges <- colSums(ActualAvailability.NoMaterialsBadge[3:4, c(2,3,4,5)])
After_badges <- c("2014-2015", After_badges)
ActualAvailability.NoMaterialsBadge <- rbind(ActualAvailability.NoMaterialsBadge, Before_badges, After_badges)

## Open dataframe "ActualAvailability.NoMaterialsBadge" for final output
